...
function CalendarApp() {
  ...
  const content = <>{!organizing && <><a className="skip-link" href="#main">Skip to content</a><header className={`site-header ${readOnly ? 'viewer-header' : ''}`}>
    <div className="header-identity"><a className="brand" href={base ? `${base}/` : workspace || '/'} onClick={e => { if (base) { e.preventDefault(); go(); } }}><CalendarDays size={readOnly ? 22 : 27} aria-hidden="true" /><span className={readOnly ? undefined : 'commons-wordmark'}>{readOnly ? config.name || 'Calendar' : 'commons'}</span></a>{readOnly && <span className="view-only-badge">View only</span>}</div>
    ...
    <footer><span>{readOnly ? 'Calendar powered by Commons' : 'A little infrastructure for being together.'}</span><div className="footer-links">{base && <button className="text-link" onClick={() => go('help')}>No account needed. · Help & privacy</button>}<a className="text-link" href="https://github.com/zendesignlabs/commons-cal" target="_blank" rel="noopener noreferrer">Source code <ArrowUpRight size={16} aria-hidden="true" /></a></div></footer>
</>;
