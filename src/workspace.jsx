...
import { CalendarDays, ArrowUpRight, Plus, ArrowLeft, Copy, Check, CircleCheck, Eye, EyeOff } from 'lucide-react';
...
function CreateWorkspace({ workspacePath = '', onCreated, onCancel }) {
  ...
  if (created) return <section className="stack workspace-created"><CircleCheck size={32} className="accent" /><h2 ref={heading} tabIndex={-1}>Your calendar is ready.</h2><p><strong>{created.name}</strong> {workspacePath ? 'has been added to this workspace. Your existing organizer link manages it, too.' : 'has its own public calendar and private organizer workspace.'}</p>{!workspacePath && <LinkToKeep sensitive label="Private organizer link" path={created.workspacePath} explanation="Save this link somewhere safe. It lets you add events and edit settings. Share it only with other organizers." />}<LinkToKeep label="Public calendar link" path={created.viewPath} explanation="Share this link with your community. Everyone can view events and subscribe; it doesn’t allow changes." /><a className="button primary" href={workspacePath ? created.calendarPath : created.workspacePath}>{workspacePath ? 'Open calendar' : 'Open your workspace'} <ArrowUpRight size={18} /></a><p className="muted small">Add your first event or fill in this calendar’s FAQ.</p><div className="actions"><Button type="button" onClick={startOver}>{workspacePath ? 'Create another calendar' : 'Create another workspace'}</Button>{onCancel && <Button type="button" onClick={onCancel}>Done</Button>}</div></section>;
...
function LinkToKeep({ label, path, explanation, sensitive = false }) {
  const [copied, setCopied] = useState(false), [message, setMessage] = useState(''), [reveal, setReveal] = useState(false);
  const ref = useRef(), id = sensitive ? 'new-private-link' : 'new-public-link';
  const value = location.origin + path;
  const copy = async () => {
    try { await navigator.clipboard.writeText(value); setCopied(true); setMessage('Link copied.'); }
    catch {
      setCopied(false);
      if (sensitive) setReveal(true);
      setMessage('Select and copy the link manually.');
      requestAnimationFrame(() => { ref.current?.focus(); ref.current?.select(); });
    }
  };
  const copyButton = <Button type="button" primary={sensitive} icon={copied ? Check : Copy} onClick={copy}>{copied ? 'Copied' : sensitive ? 'Copy organizer link' : 'Copy link'}</Button>;
  return <div className="stack created-link"><label htmlFor={id}>{label}</label><div className={`copy-row${sensitive ? ' private-link-row' : ''}`}><input id={id} ref={ref} value={sensitive && !reveal ? location.origin + '/o/••••••••••••••••/' : value} readOnly autoComplete="off" spellCheck={false} onFocus={e => { if (!sensitive || reveal) e.target.select(); }} />{sensitive ? <Button type="button" className="icon-button" icon={reveal ? EyeOff : Eye} aria-label={reveal ? 'Hide organizer link' : 'Reveal organizer link'} aria-pressed={reveal} aria-controls={id} onClick={() => setReveal(v => !v)} /> : copyButton}</div>{sensitive && copyButton}<p className="muted small">{explanation}</p><span className="small" role="status">{message}</span></div>;
}
